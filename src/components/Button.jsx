...
export default function Button({ title, icon, href }) {
  const content = (
    <>
      {icon && <Image src={icon} width={20} height={20} alt="icon" />}
      <span className={`font-medium text-[18px] ${workSans.className}`}>
        {title}
      </span>
    </>
  );
  ...
  if (href) {
    return (
      <Link
        href={href}
        className="flex items-center gap-2 bg-[#D3B86A] px-[40px] py-[12px] rounded-md w-[279px]"
      >
        {content}
      </Link>
    );
